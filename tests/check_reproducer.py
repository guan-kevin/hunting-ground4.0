...
with open('test.json') as f:
    ...
    for buildpair in data:
        for jobpair in buildpair['jobpairs']:
            jobpairs.append((buildpair['repo'], buildpair['failed_build']['build_id'], buildpair['passed_build']['build_id'], jobpair['failed_job']['job_id'], jobpair['passed_job']['job_id']))
...
for jobpair in jobpairs:
    log_dir = os.path.join('output', 'tasks', 'test', jobpair[0], '-1-{}-{}'.format(jobpair[1], jobpair[2]), '{}-{}'.format(jobpair[3], jobpair[4]))
